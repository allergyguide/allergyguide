...
export const handler: Handler = async (event) => {
  try {
    ...
    if (!foodsPath.startsWith(secureRoot)) throw new HttpError("Invalid provisioned foods path", 400);
    const protocolsPath = resolve(secureRoot, oitConfig.provisioned_protocols);
    if (!protocolsPath.startsWith(secureRoot)) throw new HttpError("Invalid provisioned protocols path", 400);
    ...
    if (err instanceof HttpError) {
      ...
      console.error("Unhandled Server Error during OIT calculator bootstrap:", err);
      return {
        statusCode: 500,
        headers: { 'Content-Type': 'application/json' },
