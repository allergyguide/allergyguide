...
export function attachKeyboardShortcuts() {
	...
	if (undoBtn)
		undoBtn.addEventListener("click", () => workspace.getActive().undo());
	if (redoBtn)
		redoBtn.addEventListener("click", () => workspace.getActive().redo());
	...
	if (container) {
		container.addEventListener("click", (e) => {
			...
			if (current && strategy && strategy !== current.dosingStrategy) {
				const updated = recalculateProtocol({
					...current,
				...
				workspace
					.getActive()
					.setProtocol(updated, `Dosing Strategy changed to: ${strategy}`);
			}
		});
	}
}
