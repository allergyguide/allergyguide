...
function formatWarningMessage(w: Warning): string {
  let msg = w.message;
  const title = w.stepIndex !== undefined && w.stepIndex !== null ? `Step ${w.stepIndex}` : "Protocol Issues";
  const prefix = `${title}: `;
 ...
 */
const renderGroup = (title: string, warnings: Warning[], severity: 'red' | 'yellow') => html`
 ...
 */
export function WarningsSidebar(warnings: Warning[], rulesURL: string): TemplateResult {
