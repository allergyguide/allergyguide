...
describe('State: ProtocolState', () => {
  ...
  const mockProtocol = (name: string) => ({
    ...
    foodA: { name, type: FoodType.SOLID, gramsInServing: new Decimal(1), servingSize: new Decimal(100), getMgPerUnit: () => new Decimal(10) },
  ...
  it('should emit correct contexts for different actions', () => {
    ...
    state.setProtocol(mockProtocol('P1'), 'Initial', { debounceHistory: false });
    expect(listener).toHaveBeenLastCalledWith(expect.anything(), "", 'structural');
