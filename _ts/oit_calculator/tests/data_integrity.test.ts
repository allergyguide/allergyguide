...
describe("Static Data Integrity", () => {
	...
	it("validates all secure_assets/oit_calculator .jsons", async (ctx) => {
		...
		console.log(
			`Found ${filePaths.length} secure json files to try validating.`,
		);
...
function validateFoodList(list: any[], filePath: string): number {
	...
	list.forEach((item, index) => {
		...
		if (!result.success) {
			console.error(
				`[${filePath}] Invalid Food at index ${index}:`,
				result.error,
			);
			errors++;
			return;
...
function validateProtocolList(list: any[], filePath: string): number {
	...
	list.forEach((item: any, index) => {
		...
		if (!result.success) {
			console.error(
				`Invalid Protocol structure at ${filePath}, index ${index}:`,
				result.error,
			);
			invalidCount++;
			return;
		...
		if (hasFoodBSteps && !p.food_b) {
			console.error(
				`Protocol "${p.name}" has Food B steps but no 'food_b' definition.`,
			);
			invalidCount++;
		}
		...
		p.table.forEach((row, rowIndex) => {
			...
			if (currentTarget.lt(prevTarget)) {
				console.error(
					`Protocol "${p.name}" Step ${rowIndex + 1}: Target ${currentTarget} is less than previous ${prevTarget}`,
				);
				invalidCount++;
			}
			...
			if (foodType === "CAPSULE" && row.method !== "CAPSULE") {
				console.error(
					`Protocol "${p.name}" Step ${rowIndex + 1}: Food ${row.food} is type CAPSULE but step method is ${row.method}.`,
				);
				invalidCount++;
			}
		});
