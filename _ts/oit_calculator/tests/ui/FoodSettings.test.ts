...
describe("FoodSettings Component", () => {
	...
	it("should render Food A protein and serving size", () => {
		renderFoodASettings(mockWorkspace, mount);
		const proteinInput = mount.querySelector(
			"#food-a-protein",
		) as HTMLInputElement;
		const servingInput = mount.querySelector(
			"#food-a-serving-size",
		) as HTMLInputElement;
	...
	it("should render Food B if present", () => {
		...
		const thresholdInput = mount.querySelector(
			"#food-b-threshold",
		) as HTMLInputElement;
	...
	it("should toggle advanced settings open state", () => {
		...
		expect(
			mockWorkspace.getActive().setAdvancedSettingsOpen,
