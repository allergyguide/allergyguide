...
describe("searchUI Navigation and Selection Logic", () => {
  ...
  const mockResults = [
    ...
    { type: "protocol", data: { name: "Peanut Protocol", dosing_strategy: "STANDARD" } }
  ];
  ...
  beforeEach(() => {
    ...
    searchUI.showSearchDropdown("food-a-search", mockResults as any, "testQuery", mockCallbacks as any);
  });
  ...
  it("navigateDropdown('down') should cycle from -1 to 0", () => {
    ...
    expect(aMount.querySelector('[data-index="0"]')?.classList.contains("highlighted")).toBe(true);
  });
  ...
  it("navigateDropdown('up') from -1 should loop to last item (Custom Food)", () => {
    ...
    expect(aMount.querySelector('[data-index="2"]')?.classList.contains("highlighted")).toBe(true);
    expect(aMount.querySelector('[data-index="2"]')?.textContent).toContain("Create Custom Food:");
  });
  ...
  it("selectHighlightedDropdownItem should call onSelectCustom for last index", () => {
    ...
    expect(mockCallbacks.onSelectCustom).toHaveBeenCalledWith("testQuery", "food-a-search");
  });
  ...
  it("selectHighlightedDropdownItem should call correct action for result indices", () => {
    ...
    searchUI.selectHighlightedDropdownItem();
    expect(mockCallbacks.onSelectFoodA).toHaveBeenCalledWith(mockResults[0].data);

    // Index 1: Peanut Protocol (protocol)
    searchUI.showSearchDropdown("food-a-search", mockResults as any, "testQuery", mockCallbacks as any); // Reset activeIndex
    searchUI.navigateDropdown("down");
    searchUI.navigateDropdown("down"); // to index 1
    searchUI.selectHighlightedDropdownItem();
    expect(mockCallbacks.onSelectProtocol).toHaveBeenCalledWith(mockResults[1].data);
  });
});
