...
const createFood = (
	type: FoodType = FoodType.SOLID,
	proteinPercent: number = 10,
): Food => ({
	...
	describe("findDilutionCandidates", () => {
		it("should find candidates for solid foods within constraints", () => {
			...
			candidates.forEach((c) => {
				expect(c.mixFoodAmount.toNumber()).toBeGreaterThanOrEqual(
					DEFAULT_CONFIG.minMeasurableMass.toNumber(),
				);
				expect(c.mixWaterAmount.toNumber()).toBeGreaterThanOrEqual(
					DEFAULT_CONFIG.minMeasurableVolume.toNumber(),
				);
				expect(c.mixWaterAmount.toNumber()).toBeLessThanOrEqual(
					DEFAULT_CONFIG.MAX_MIX_WATER.toNumber(),
				);
				...
				expect(diff.toNumber()).toBeLessThanOrEqual(
					DEFAULT_CONFIG.PROTEIN_TOLERANCE.toNumber(),
				);
			});
		});
		...
		it("should find candidates for liquid foods using additive volume", () => {
			...
			candidates.forEach((c) => {
				...
				expect(c.mixTotalVolume.minus(c.mixFoodAmount).toNumber()).toBeCloseTo(
					c.mixWaterAmount.toNumber(),
				);
			...
			if (candidates.length >= 2) {
				...
				if (conc.greaterThan(DEFAULT_CONFIG.MAX_SOLID_CONCENTRATION)) {
					...
					const anyMet = candidates.some((c) =>
						c.mixFoodAmount
							.dividedBy(c.mixWaterAmount)
							.lessThanOrEqualTo(DEFAULT_CONFIG.MAX_SOLID_CONCENTRATION),
					);
					// If some met it, but the first one didn't, that's a failure.
					expect(anyMet).toBe(false);
				} else {
					expect(conc.toNumber()).toBeLessThanOrEqual(
						DEFAULT_CONFIG.MAX_SOLID_CONCENTRATION.toNumber(),
					);
				}
			}
		});
		...
		it("should respect PROTEIN_TOLERANCE", () => {
			...
			const looseCandidates = findDilutionCandidates(
				new Decimal(13),
				solidFood,
				looseConfig,
			);
		...
		describe("Water Resolution Snapping (0.5ml increments)", () => {
			...
			it("should return ONLY 0.5ml aligned water amounts when they satisfy tolerance", () => {
				...
				const matches = candidates.filter(
					(c) => c.mixFoodAmount.equals(1) && c.dailyAmount.equals(1),
				);
				...
				const hasMessy = matches.some(
					(m) => !m.mixWaterAmount.mod(0.5).equals(0),
				);
	...
	describe("generateDefaultProtocol", () => {
		it("should generate a full protocol", () => {
			...
			for (let i = 1; i < protocol.steps.length; i++) {
				expect(protocol.steps[i].targetMg.toNumber()).toBeGreaterThan(
					protocol.steps[i - 1].targetMg.toNumber(),
				);
			}
		});
		...
		it("should provide fallback DIRECT steps if dilution fails", () => {
			...
			const expectedMass = firstStep.targetMg.dividedBy(
				solidFood.getMgPerUnit(),
			);
			expect(firstStep.dailyAmount.toNumber()).toBeCloseTo(
				expectedMass.toNumber(),
			);
		});
	});
});
