...
describe("Core: Protocol Manipulation", () => {
	...
	describe("addFoodBToProtocol", () => {
		it("should transition to Food B at correct threshold", () => {
			...
			expect(transitionStep.targetMg.toNumber()).toEqual(
				prevStep.targetMg.toNumber(),
			);
	...
	describe("Step Updates", () => {
		it("updateStepTargetMg should recalculate daily amount for DIRECT steps", () => {
			// Find a direct step
			const index =
				baseProtocol.steps.findIndex((s) => s.method === Method.DIRECT) + 1;
		...
		it("updateStepTargetMg should snap DIRECT amounts if within tolerance", () => {
			...
			const index =
				proto.steps.findIndex((s) => s.method === Method.DIRECT) + 1;
		...
		it("updateStepTargetMg should recalculate servings/water for DILUTE steps", () => {
			// Find dilute step
			const index =
				baseProtocol.steps.findIndex((s) => s.method === Method.DILUTE) + 1;
			...
			expect(newStep.mixFoodAmount?.toNumber()).toBe(
