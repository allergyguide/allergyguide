...
describe("Core: Validator", () => {
	...
	describe("Critical Errors (Red)", () => {
		it("should flag TOO_FEW_STEPS for too few steps", () => {
			...
			expect(
				warnings.some((w) => w.code === WarningCode.Red.TOO_FEW_STEPS),
			).toBe(true);
		});
		...
		it("should flag PROTEIN_MISMATCH for protein mismatch > tolerance", () => {
			...
			const warnings = validateProtocol(protocol);
			expect(
				warnings.some((w) => w.code === WarningCode.Red.PROTEIN_MISMATCH),
			).toBe(true);
		});
		...
		it("should flag IMPOSSIBLE_VOLUME for impossible volumes (mix < daily)", () => {
			...
			const protocol = generateDefaultProtocol(food, DEFAULT_CONFIG);
			const diluteStepIndex = protocol.steps.findIndex(
				(s) => s.method === Method.DILUTE,
			);
			...
			const warnings = validateProtocol(protocol);
			expect(
				warnings.some((w) => w.code === WarningCode.Red.IMPOSSIBLE_VOLUME),
			).toBe(true);
		});
		...
		it("should flag INVALID_CONCENTRATION for invalid food protein concentration", () => {
			...
			expect(
				warnings.some(
					(w) =>
						w.code === WarningCode.Red.INVALID_CONCENTRATION &&
						w.message.includes("protein concentration must be > 0"),
				),
			).toBe(true);
		});
		...
		it("should flag INVALID_CONCENTRATION when protein content is greater than serving size", () => {
			const badFood = {
				...food,
				gramsInServing: new Decimal(150),
				servingSize: new Decimal(100),
			}; // 150g protein in 100g serving
			const protocol = generateDefaultProtocol(badFood, DEFAULT_CONFIG);
			const warnings = validateProtocol(protocol);
			expect(
				warnings.some(
					(w) =>
						w.code === WarningCode.Red.INVALID_CONCENTRATION &&
						w.message.includes(
							"protein amount cannot be greater than a serving size of",
						),
				),
			).toBe(true);
		});
		...
		it("should flag INVALID_TARGET for zero or negative targetMg", () => {
			...
			expect(
				warnings.some((w) => w.code === WarningCode.Red.INVALID_TARGET),
			).toBe(true);
		...
		it("should flag INSUFFICIENT_MIX_PROTEIN when dilution mix has less protein than target", () => {
			...
			expect(
				warnings.some(
					(w) => w.code === WarningCode.Red.INSUFFICIENT_MIX_PROTEIN,
				),
			).toBe(true);
		});
		...
		it("should flag INVALID_DILUTION_STEP_VALUES for negative physical values", () => {
			...
			expect(
				warnings.some(
					(w) => w.code === WarningCode.Red.INVALID_DILUTION_STEP_VALUES,
				),
			).toBe(true);
	...
	describe("Warnings (Yellow)", () => {
		it("should flag LOW_SERVINGS for low servings", () => {
			...
			const warnings = validateProtocol(protocol);
			expect(
				warnings.some((w) => w.code === WarningCode.Yellow.LOW_SERVINGS),
			).toBe(true);
		});
		...
		it("should flag NON_ASCENDING_STEPS for non-ascending targets", () => {
			...
			const warnings = validateProtocol(protocol);
			expect(
				warnings.some((w) => w.code === WarningCode.Yellow.NON_ASCENDING_STEPS),
			).toBe(true);
		});
		...
		it("should flag BELOW_RESOLUTION for measurements below tool resolution", () => {
			...
			const directIndex = protocol.steps.findIndex(
				(s) => s.method === Method.DIRECT,
			);
			protocol.steps[directIndex].dailyAmount = new Decimal(0.01); // < 0.2g default min

			const warnings = validateProtocol(protocol);
			expect(
				warnings.some((w) => w.code === WarningCode.Yellow.BELOW_RESOLUTION),
			).toBe(true);
		});
		...
		it("should flag HIGH_SOLID_CONCENTRATION for high solid concentration in liquid", () => {
			...
			expect(
				warnings.some(
					(w) => w.code === WarningCode.Yellow.HIGH_SOLID_CONCENTRATION,
				),
			).toBe(true);
		});
		...
		it("should flag DUPLICATE_STEP for redundant adjacent steps", () => {
			...
			const warnings = validateProtocol(protocol);
			expect(
				warnings.some((w) => w.code === WarningCode.Yellow.DUPLICATE_STEP),
			).toBe(true);
		});
		...
		it("should flag HIGH_DAILY_AMOUNT when daily amount exceeds limit", () => {
			...
			const warnings = validateProtocol(protocol);
			expect(
				warnings.some((w) => w.code === WarningCode.Yellow.HIGH_DAILY_AMOUNT),
			).toBe(true);
		});
		...
		it("should flag HIGH_MIX_WATER when mix water exceeds limit", () => {
			...
			const warnings = validateProtocol(protocol);
			expect(
				warnings.some((w) => w.code === WarningCode.Yellow.HIGH_MIX_WATER),
			).toBe(true);
		});
		...
		it("should flag RAPID_ESCALATION for >2x increase in dose", () => {
			...
			const warnings = validateProtocol(protocol);
			expect(
				warnings.some((w) => w.code === WarningCode.Yellow.RAPID_ESCALATION),
			).toBe(true);
		});
		...
		it("should flag NO_TRANSITION_POINT when Food B is defined but no transition steps exist", () => {
			...
			const warnings = validateProtocol(protocol);
			expect(
				warnings.some((w) => w.code === WarningCode.Yellow.NO_TRANSITION_POINT),
			).toBe(true);
		});
