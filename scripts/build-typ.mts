...
export function loadTypstBinary() {
	try {
		...
		if (!existsSync(typstBin) && isLinux) {
			...
			execSync(
				`curl -L -o ${FILENAME} https://github.com/typst/typst/releases/download/v${TYPST_VERSION}/typst-x86_64-unknown-linux-musl.tar.xz`,
			);

			// Verify Checksum (Shell out to sha256sum for simplicity)
			console.log("Verifying checksum...");
			const calculatedHash = execSync(
			...
			if (calculatedHash !== TYPST_HASH) {
				throw new Error(
					`Checksum mismatch! Expected ${TYPST_HASH} but got ${calculatedHash}`,
				);
			}

			console.log("Checksum verified. Extracting...");
			execSync(
				`tar -xf ${FILENAME} --strip-components=1 --wildcards '*/typst'`,
			);
