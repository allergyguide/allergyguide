...
async function abridge() {
	...
	if (search_library === "offline") {
		replaceInFileSync({
			files: "config.toml",
			from: /index_format.*=.*/g,
			to: 'index_format = "elasticlunr_javascript"',
		});
	...
	if (search_library === "pagefind") {
		...
		replaceInFileSync({
			files: path.join(__dirname, "static/js/pagefind_search.js"),
			from: /pagefind-entry\.json\?ts=/g,
			to: "pagefind-entry-" + hash + "\.json",
		});
		replaceInFileSync({
			files: path.join(__dirname, "static/js/pagefind_search.js"),
			from: /Date.now\(\)/g,
		...
		_cpRecursive(
			path.join(__dirname, "static/js/index"),
			path.join(__dirname, "public/js/index"),
		);
		_cpRecursive(
			path.join(__dirname, "static/js/fragment"),
			path.join(__dirname, "public/js/fragment"),
		);
	}

	if (pwa) {
		// Update pwa settings, file list, and hashes.
		if (
			...
			if (fs.existsSync("static/js/sw_load.js")) {
				...
				replaceInFileSync({
					files: "static/js/sw_load.js",
					from: /sw.*v=.*/g,
					to: "sw" + sw_load_min + pwa_VER + '",',
				});
			}
			if (fs.existsSync("static/sw.js")) {
				replaceInFileSync({
					files: "static/sw.js",
					from: /NORM_TTL.*=.*/g,
					to: "NORM_TTL = " + pwa_NORM_TTL + ";",
				});
				replaceInFileSync({
					files: "static/sw.js",
					from: /LONG_TTL.*=.*/g,
					to: "LONG_TTL = " + pwa_LONG_TTL + ";",
				});
				replaceInFileSync({
					files: "static/sw.js",
					from: /TTL_NORM.*=.*/g,
					to: "TTL_NORM = [" + pwa_TTL_NORM + "];",
				});
				replaceInFileSync({
					files: "static/sw.js",
					from: /TTL_LONG.*=.*/g,
					to: "TTL_LONG = [" + pwa_TTL_LONG + "];",
				});
				replaceInFileSync({
					files: "static/sw.js",
					from: /TTL_EXEMPT.*=.*/g,
					to: "TTL_EXEMPT = [" + pwa_TTL_EXEMPT + "];",
				});
			}

			if (pwa_cache_all === true) {
				console.log(
					"info: pwa_cache_all = true in config.toml, so caching the entire site.\n",
				);
				...
				cache = "";
				files = fs
					.readdirSync(path, { recursive: true, withFileTypes: false })
					.forEach((file) => {
	...
	if (fs.existsSync("static/manifest.json")) {
		...
		try {
			out = JSON.minify(
				fs.readFileSync("static/manifest.json", { encoding: "utf-8" }),
			);
		} catch (err) {
			console.log(err);
		}
	...
	console.log(
		"Zola Build to generate new integrity hashes for the previously minified files:",
	);
	await execWrapper("zola build" + args);
}
...
async function _headersWASM() {
	...
	if (search_library === "pagefind") {
		replaceInFileSync({
			files: "static/_headers",
			from: /script-src 'self'/g,
			to: "script-src 'wasm-unsafe-eval' 'self'",
		});
	} else if (search_library === "tinysearch") {
		replaceInFileSync({
			files: "static/_headers",
		...
		replaceInFileSync({
			files: "static/_headers",
			from: /script-src 'wasm-unsafe-eval' 'self'/g,
			to: "script-src 'self'",
		});
	}
}
...
function _rmRecursive(targetFiles) {
	try {
		...
		if (error.code !== "ENOENT") {
			// Ignore if does not exist, that is the desired result.
...
function _cpRegex(source, dest, regex) {
	try {
		fs.readdirSync(source)
			.filter((f) => regex.test(f))
			.forEach((f) => fs.copyFileSync(source + f, dest + f));
	} catch (error) {
		console.error("An error occurred:", error);
	}
}
...
function bundle(
	...
	if (search_library) {
		if (
			...
			(search_library === "elasticlunrjava" && uglyurls === true)
		) {
...
function minify(fileA, outfile) {
	...
	if (!outfile) {
		// outfile parameter omitted, infer based on input
		outfile = fileA[0].slice(0, -2) + "min.js";
	}
	var filesContents = fileA.map(function (file) {
		// array input to support multiple files
...
async function searchChange(searchOption) {
	...
	replaceInFileSync({
		files: "config.toml",
		from: /search_library.*=.*/g,
		to: 'search_library = \"' + searchOption + '\"',
	});
}
...
async function createPagefindIndex() {
	...
	await Promise.all(promises)
		...
		.then(async () => {
			...
			const pagefind_searchPath = path.join(
				bpath,
				"static/js/pagefind.search.js",
			); //file to insert into
			const search_pagefindPath = path.join(
				__dirname,
				"static/js/pagefind_search.js",
			); //output
			...
			pagefind_searchContent = pagefind_searchContent.replace(
				/\/\/insertHere/g,
				pagefindContent,
			);
