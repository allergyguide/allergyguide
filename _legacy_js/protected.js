...
class ProtectedContentLoader {
			...
			if (!response.ok) {
				...
				try {
					...
					if (
						...
						throw new Error(
							`Function returned HTML instead of JSON. Status: ${response.status}. This usually means the function isn't deployed or there's a server error.`,
						);
					}
	...
	async processImages(content, token, basePath = "") {
		...
		const imagePromises = images.map(async (img) => {
			...
			this.log(
				`Image fetch result for ${img.srcPath}:`,
...
window.logoutProtectedContent = function () {
	...
	protectedElements.forEach((element) => {
		element.innerHTML =
			'<div class="protected-logged-out">Logged out. <a href="#" class="reload-link">Reload</a> to access content.</div>';
