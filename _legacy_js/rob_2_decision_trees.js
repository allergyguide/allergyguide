...
export function traverseTree(node, answers) {
	...
	const branch = node.answers.find((branch) =>
		branch.accepted.includes(userAnswer),
	);
	if (!branch) {
		return "null";
	}
...
export function getDomainAnswers(domain, num_q) {
	...
	for (let i = 1; i <= num_q; i++) {
		...
		const rawText =
			document.querySelector(radioSelector)?.nextElementSibling.innerText;
	...
	answers: [
		{
			...
			next: {
				...
				evaluate: (answers) => {
					const goodpath = [Answer.NO, Answer.PNO];
					if (
						goodpath.includes(answers.q2b_4) &&
						goodpath.includes(answers.q2b_5)
					) {
						return "good";
					}
					return "bad";
...
export const D5_DT = {
	...
	evaluate: (answers) => {
		if (
			[Answer.YES, Answer.PYES].includes(answers.q5_2) ||
			[Answer.YES, Answer.PYES].includes(answers.q5_3)
		) {
			return "high";
		} else if (
			[Answer.NO, Answer.PNO].includes(answers.q5_2) &&
			[Answer.NO, Answer.PNO].includes(answers.q5_3)
		) {
			return "low";
