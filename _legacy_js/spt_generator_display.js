...
export function getTable(entryList, useCol, sort) {
	...
	for (let i = 0; i < rows; i++) {
		...
		const numNoteLines = Math.max(
			leftCell.noteLines.length,
			rightCell.noteLines.length,
		);
		for (let j = 0; j < numNoteLines; j++) {
			const leftNote = leftCell.noteLines[j] || "";
			const rightNote = rightCell.noteLines[j] || "";
			let noteLine =
				leftNote.padEnd(fixedWidth) + separator + rightNote.padEnd(fixedWidth);
...
export function getSummary(entryList) {
	...
	const sortedPositiveKeys =
		Object.keys(positiveAllergens).sort(compareAllergens);
	const sortedNegativeAllergens =
		Array.from(negativeAllergens).sort(compareAllergens);
