import {
	...
	D4_DT,
	D5_DT,
} from "./rob_2_decision_trees.js";
import {
	getAnswer,
	setup_save_state,
	DomainRisk,
	Answer,
	getDomainAnswerDetails,
	getMarkdownSummary,
	get_admon_class,
} from "./rob_2_utils.js";
...
function getOverallSummary(button) {
	...
	for (let domain of domains) {
		try {
			var admonTextElement = document.querySelector(
				`.admon_macro-${domain} .admonition-content`,
			);
			var admonTitleElement = document.querySelector(
				`.admon_macro-${domain} .admonition-title`,
			);
...
const process_dt = (domain, num_q, tree) => {
	...
	var admon_text = document.querySelectorAll(
		`.admon_macro-${domain} .admonition-content`,
	)[0];
	var admon_title = document.querySelectorAll(
		`.admon_macro-${domain} .admonition-title`,
	)[0];
	var admon = document.querySelectorAll(
		`.admon_macro-${domain} .admonition`,
	)[0];
...
const process_dt2 = () => {
	...
	if (outcome_p1 == DomainRisk.Low && outcome_p2 == DomainRisk.Low) {
		...
		outcome_p1 == DomainRisk.Concerns ||
		outcome_p2 == DomainRisk.Concerns
	) {
...
function add_radio_text_listeners(domain, num_q, tree) {
	document
		.querySelectorAll(`input[type="radio"][name^="q${domain}_"]`)
		.forEach((radio) => {
	...
	document
		.querySelectorAll(`textarea.question-details.q${domain}`)
		.forEach((textarea) => {
...
document.addEventListener("DOMContentLoaded", () => {
	...
	document
		.querySelectorAll('input[type="radio"][name^="q2b_"]')
		.forEach((radio) => {
			radio.addEventListener("click", d2b_updateQuestionVisibility);
		});

	document
		.querySelectorAll('input[type="radio"][name^="q3_"]')
		.forEach((radio) => {
			radio.addEventListener("click", d3_updateQuestionVisibility);
		});

	document
		.querySelectorAll('input[type="radio"][name^="q4_"]')
		.forEach((radio) => {
...
document
	.getElementById("rob2-clear-button")
	.addEventListener("click", clearAllState);
...
function d2_updateQuestionVisibility() {
	...
	const showQ23 =
		[Answer.YES, Answer.PYES, Answer.NOINFO].includes(answerQ21) ||
		[Answer.YES, Answer.PYES, Answer.NOINFO].includes(answerQ22);
	const showQ24 = showQ23 && [Answer.YES, Answer.PYES].includes(answerQ23);
	const showQ25 =
		showQ24 && [Answer.YES, Answer.PYES, Answer.NOINFO].includes(answerQ24);
...
function d2b_updateQuestionVisibility() {
	...
	// Determine if q2_3 should be shown:
	const showQ23b =
		[Answer.YES, Answer.PYES, Answer.NOINFO].includes(answerQ21b) ||
		[Answer.YES, Answer.PYES, Answer.NOINFO].includes(answerQ22b);
	const showQ26b =
		[Answer.NO, Answer.PNO, Answer.NOINFO].includes(answerQ23b) ||
		[Answer.YES, Answer.PYES, Answer.NOINFO].includes(answerQ24b) ||
		[Answer.YES, Answer.PYES, Answer.NOINFO].includes(answerQ25b);
...
function d4_updateQuestionVisibility() {
	...
	// Determine if q2_3 should be shown:
	const showQ43 =
		[Answer.NO, Answer.PNO, Answer.NOINFO].includes(answerQ41) &&
		[Answer.NO, Answer.PNO, Answer.NOINFO].includes(answerQ42);
