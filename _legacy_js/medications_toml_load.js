document.addEventListener("DOMContentLoaded", function () {
	...
	const cardData = drugCards.map((card) => {
		...
		return {
			...
			categories: card.dataset.categories.split(",").map((c) => c.trim()),
		};
	});
	...
	filterButtons.forEach((button) => {
		button.addEventListener("click", () => {
			...
			if (category === "all") {
				...
				if (anyActive) {
					document
						.querySelector('.filter-button[data-category="all"]')
	...
	function updateCards() {
		...
		if (searchQuery === "") {
			cardData.forEach((card) => {
				let categoryMatches =
					...
					activeCategories.every((cat) => card.categories.includes(cat));
		...
		cardData.forEach((card) => {
			...
			card.brandNamesArray.forEach((brandName) => {
				const brandResult = fuzzysort.single(searchQuery, brandName);
				if (brandResult) {
					if (bestScore === null || brandResult.score > bestScore) {
