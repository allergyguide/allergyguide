...
function createInputComponents() {
	...
	// Diameter Input
	const diameterInput = createNumberInput(
		"Biggest dimension in mm",
...
function handleAllergenSearch(e) {
	...
	if (results.length) {
		results.forEach((result) => {
			...
			const highlighted = result.highlight(
...
function handleDropdownNavigation(e) {
	...
	if (
		e.key === "Tab" &&
		items.length === 1 &&
		items[0].getAttribute("data-custom") === "true"
	) {
		dropdown.classList.remove("visible");
...
function handleEntrySubmission(e) {
	if (e.key === "Enter") {
		...
		const inputs = Array.from(
			e.target.closest(".input-row").querySelectorAll("input"),
		);
		...
		entries.push(
			new Entry(normalizeAllergenName(allergen), diameter, note || null),
		);
		renderEntries();
		updateDisplays();
		inputs.forEach((input) => {
...
function handleEntryUpdate(e) {
	if (e.target.tagName === "INPUT") {
		const index = Array.from(
			e.target.closest(".entry").parentNode.children,
		).indexOf(e.target.closest(".entry"));
		const field = Array.from(e.target.parentNode.children).indexOf(e.target);

		switch (field) {
			case 0:
				entries[index].allergen = e.target.value;
				break;
			case 1:
				entries[index].diameter = parseFloat(e.target.value) || NaN;
				break;
			case 2:
				entries[index].note = e.target.value.trim() || null;
				break;
		}
...
function updateDisplays() {
	...
	const summaryCopyBtn =
		paragraphDisplay.parentElement.querySelector(".copy-button");
	if (summaryText.trim() === "") {
