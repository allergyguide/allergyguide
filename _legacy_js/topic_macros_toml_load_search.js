...
document.addEventListener("DOMContentLoaded", function () {
	...
	const macroBlocks = Array.from(document.querySelectorAll(".macro-block")).map(
		(el) => {
			const topic = el.getAttribute("data-topic") || "";
			const keywords = el.getAttribute("data-keywords") || "";
			// Split keywords into array for individual matching
			const keywordsArray = keywords.split(/\s+/).filter((k) => k.length > 0);
			return {
				el: el,
				topic: topic,
				keywords: keywords,
		...
		macroBlocks.forEach((block) => {
			block.keywordsArray.forEach((keyword) => {
				...
				if (result && result.score > -10000) {
					...
					// Keep the best score
					if (currentScore === undefined || boostedScore > currentScore) {
